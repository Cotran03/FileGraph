...
FileGraph는 로컬 파일의 출처, 사용처, 의존성과 논리적 관계를 추적하는 Windows 데스크톱 앱입니다.

기존 파일시스템을 그대로 유지하면서 별도의 관계 계층을 저장합니다. 사용자는 관계를 직접 만들 수 있고, FileGraph는 지원되는 파일을 분석해 설명 가능한 관계 후보를 제안합니다.
...
- 자동 분석 결과는 바로 확정하지 않고 신뢰도와 근거를 가진 후보로 저장한 뒤 사용자가 승인하거나 거절한다.
- 선택한 파일을 중심으로 출처, 의존 파일, 사용처와 기타 관계를 구분해 보여준다.

## 구현된 기능

수동 그래프 관리 기반 위에 파일 관계·출처·의존성 추적 흐름을 추가하고 있습니다.
...
- 관계 생성 출처(`MANUAL`, `CONFIRMED`, `SYSTEM`), 탐지 신뢰도와 근거를 저장한다.
- Python 파일의 명시적인 로컬 파일 읽기·쓰기 호출을 분석해 `읽음(READS)`, `생성/기록(WRITES)` 후보를 만든다.
- 같은 폴더의 동일한 이름을 가진 문서와 PDF를 수정 시각 근접도와 함께 `내보낸 파일(EXPORTED_AS)` 후보로 제안한다.
- 폴더/파일 등록이 끝나면 새로 등록된 파일을 자동 분석하고 발견한 후보 개수를 바로 표시한다.
- 오른쪽 패널에서 관계 후보의 신뢰도와 근거를 확인하고 승인하거나 거절한다.
- 오른쪽 패널의 `파일 맥락`, `관계`, `후보` 탭으로 상세 영역을 전환해 작은 창에서도 내용이 잘리지 않게 확인한다.
- 일반 데이터 변경과 새로고침은 사용자가 보고 있던 그래프 중심과 확대 비율을 유지한다.
- 추천 관계를 승인하면 새 관계의 두 파일이 있는 위치로 화면 중심이 자동 이동한다.
- 컨트롤 패널은 400px 고정 폭으로 유지되어 창이나 분할 영역을 조절해도 잘리지 않는다.
- 선택 파일의 출처, 의존 파일, 이 파일을 사용하는 파일을 파일 맥락 영역에서 확인한다.
- `영향 보기`로 선택 파일에서 관계 방향을 따라 잠재적으로 영향받는 downstream 파일을 확인한다.
- 그래프에서 노드를 드래그해 배치하고 좌표를 저장한다.
- 자동 정렬은 노드 사이 여백과 노드-간선 충돌을 후처리해 겹침을 줄인다.
- 노드 30~220개의 연결이 많은 그래프는 여러 spring 배치 후보 중 간선 교차와 노드-간선 충돌 점수가 가장 낮은 결과를 선택한다.
- 같은 두 노드 사이에 여러 관계가 있으면 간선을 평행하게 분리해 각 관계를 구분한다.
- 파일 확장자, 폴더, 누락, 접근 거부 상태별 SVG 아이콘을 표시한다.
- 설정 화면에서 확장자별 아이콘 매핑을 직접 추가/수정/삭제할 수 있다.
- 폴더 노드의 내부 파일 노드를 우클릭 메뉴에서 접거나 펼칠 수 있다.
- 접은 폴더를 이동한 뒤 펼치면 하위 파일의 상대 배치를 새 폴더 위치 기준으로 함께 이동한다.
...
다음 단계는 탐지기 지원 범위 확대, 후보 일괄 검토, 상·하류 의존성 순회와 영향 보기, 더 정교한 파일 이동 추적입니다.
