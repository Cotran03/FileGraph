...
- `gui/main_window.py`: 메인 윈도우, 툴바, 검색, 그래프/제어 패널 연결, 파일 상태 확인, 누락 찾기, 폴더 내부 파일 접기/펼치기
- `gui/graph_viewer.py`: 그래프 렌더링, 확장자/상태별 SVG 아이콘 표시, 드래그, 선택, 라벨, 컨텍스트 메뉴, 드롭 처리
- `gui/control_panel.py`: 사이드 패널, 관계 목록, 관계 수정/삭제, 포커스/전체 보기, 글자 크기, 상태 확인/누락 찾기 버튼
...
- 폴더 내부 파일 재귀 등록 옵션과 `CONTAINS` 관계 자동 생성
- 기본 관계 타입 seed와 관계 추가/수정/삭제
- 관계 방향성, 강도, 설명 저장
- 그래프 렌더링, 자동 배치, 노드 드래그 좌표 저장
- 파일 확장자/폴더/누락/권한 상태별 SVG 아이콘 표시
- 폴더 노드의 내부 파일 접기/펼치기
- 검색, 포커스 뷰, 전체 보기, 자동 정렬
- 우클릭 컨텍스트 메뉴, 관계 목록 편집, 다중 선택 이동/삭제
...
현재 자동 테스트는 pytest 기반이며, DB/그래프/GUI/파일 무결성 흐름을 검증합니다. 2026-07-03 기준 전체 테스트는 91개입니다.
...
12. 파일 상태 확인과 해시 기반 누락 파일 복구
13. 확장자/상태별 아이콘 표시
14. 폴더 내부 파일 접기/펼치기

다음 우선순위:

1. 다중 노드 삭제 UX와 빌드 앱 기준 QA 보강
2. 폴더 등록 시 무시할 폴더 정책과 설정 UI 보강
3. 설정 탭 추가와 설정 화면 확장 기반 마련
4. 노드/간선 라벨 위치와 가독성 개선
5. 노드 겹침 방지 레이아웃 보정
6. 컨트롤 패널 액션 문구와 관계 확인 UI 개선
7. 기본 단축키 추가
8. 상단 툴바 제거와 컨트롤 패널 중심 액션 정리
9. 관계 타입 관리 화면
10. 파일 이동 추적 정책 보강
11. scan_history 기록과 스캔 결과 화면
12. AI 관계 추천
13. AI 폴더 정리와 Undo

다음 변경 메모:

- 설정 탭을 별도로 만든다. 이후 계속 추가될 옵션을 담을 확장 지점으로 사용하며, 초기에는 무시 폴더 목록, 그래프 기본값, 노드/간선 라벨 표시 방식, AI 사용 여부/모델명, API 키 저장 상태 안내 같은 항목을 수용할 수 있게 설계한다.
- 노드 이름과 간선 이름은 각각 `항상 보이기`와 `마우스가 올라가면 보이기`를 선택할 수 있게 한다. 두 설정은 서로 독립적으로 관리한다.
- 노드/간선 라벨 위치는 설정 항목이 아니라 기본 UI 품질로 개선한다. 노드 라벨은 노드와 겹치지 않고 주변 노드/간선과 덜 충돌하도록 배치하고, 간선 라벨은 선 중앙에서 약간 떨어져 읽기 좋은 위치에 두는 방향으로 조정한다.
- 자동 정렬 후 노드끼리 겹치지 않도록 레이아웃 후처리 보정을 추가한다. 노드 반지름과 최소 여백을 기준으로 가까운 노드를 밀어내되, 그래프 전체 형태가 과하게 무너지지 않게 한다.
- 수동 드래그로 노드끼리 겹치는 위치에 놓이면, 드롭 직후 가장 가까운 비겹침 위치로 보정해 저장한다. 여러 노드를 함께 이동할 때는 선택 집합의 상대 위치를 최대한 유지한다.
- 컨트롤 패널의 `상태 확인` 문구는 기능 의미가 애매하다. 파일 존재 여부, 접근 권한, 해시 갱신을 확인한다는 뜻이 드러나도록 버튼 문구나 보조 설명을 다듬는다.
- 컨트롤 패널 하단의 관계 확인 UI를 개선한다. 선택 노드와 연결된 관계를 더 쉽게 읽을 수 있도록 출발/도착, 관계 타입, 방향성, 강도를 구분해 표시하고, 수정/삭제 액션이 어떤 관계에 적용되는지 명확하게 한다.
- 기본 단축키를 추가한다. 우선 `Ctrl+A`는 현재 그래프에 보이는 모든 노드를 선택하는 기능으로 둔다.
- 설정 탭에 실제 API 키 원문을 표시하거나 저장하지 않는다. API 키 입력/삭제 UI가 들어가더라도 저장은 `keyring`을 사용하고, 설정 파일에는 상태나 모델명 같은 비밀값이 아닌 정보만 둔다.
- 상단 툴바와 컨트롤 패널에 중복되는 기능이 있다. 상단 툴바 아이콘만으로는 기능 구분이 어렵기 때문에, 다음 변경에서는 상단 툴바를 제거하고 주요 액션을 컨트롤 패널에 통합한다.
- 다중 선택 삭제는 현재 그래프에서 여러 노드를 선택한 뒤 삭제 대상 노드 컨텍스트 메뉴를 열어야 한다. 다음 변경에서는 컨트롤 패널에 선택 노드 개수와 일괄 삭제 버튼을 노출해, 특정 노드를 다시 클릭하지 않아도 선택 집합을 삭제할 수 있게 한다.
- 노드 우클릭 삭제 흐름은 기존 빠른 작업 경로로 유지한다. 컨트롤 패널 삭제는 더 명확한 주 진입점으로 보강한다.
- 폴더 내부 등록 시 상위 폴더 아래에 하위 폴더 하나만 있고 그 안에 파일이 있는 구조에서도 상위 폴더 노드가 그래프에 남고, 상위 폴더-하위 폴더-파일 계층이 보이도록 등록/포커스/포함 관계 흐름을 점검한다.
- 폴더 등록 시 내부 파일을 함께 등록할 때 무시할 폴더 목록을 정리한다. 기본값은 `.git`, `.venv`, `venv`, `ENV`, `node_modules`, `__pycache__`로 시작하고, 이후 `config/settings.json` 또는 설정 UI에서 사용자 지정 제외 폴더를 관리할 수 있게 한다.
- 무시 폴더 정책은 폴더 드롭/폴더 추가의 내부 파일 등록 흐름에 우선 적용하고, 필요하면 누락 파일 찾기 검색 제외 정책과 공통화한다.
