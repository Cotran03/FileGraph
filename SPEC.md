...
- `gui/settings_dialog.py`: 그래프 표시, 노드 색상 미리보기/초기화, 강조색, 확장자 아이콘 매핑, 무시 폴더 설정 화면
...
- 별도 설정 화면, 노드 타입별/hover 라벨 표시 방식 설정, 간선 라벨 표시 방식 설정, 색상 미리보기/초기화, 강조/아이콘/무시 폴더 UI
...
- 하단 상태 표시줄
- 작업 패널 주요 동작: 파일/폴더 추가, 검색, 보기 프리셋, 관계 추가, 새로고침, 파일 위치 갱신, 샘플, 자동 정렬, 누락 파일 찾기, DB 가져오기
- 설정 화면 주요 동작: 무시 폴더 목록, 그래프 라벨 기본값, 노드 색상 미리보기/초기화, 강조 색상, 확장자 아이콘 매핑
...
- 강조 색상 슬롯
- 확장자 아이콘 사용자 지정 매핑
- 설정 화면 색상 미리보기와 파일/폴더/강조 색상 기본값 초기화
...
- 작업 패널/컨트롤 바가 작은 창 높이에서 빽빽해 UI가 겹칠 수 있다. 패널 본문을 스크롤 영역으로 감싸고 버튼 밀도와 섹션 높이를 조정한다.
...
- 설정 화면에 색상 미리보기 칩과 파일/폴더/강조 색상 기본값 초기화를 추가했다.
