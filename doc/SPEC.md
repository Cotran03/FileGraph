...
- 노드 30~220개의 연결이 많은 그래프 자동 정렬은 서로 다른 seed의 spring 배치 3개를 계산하고, 간선 교차 수와 노드-간선 근접 페널티가 가장 낮은 후보를 선택한다. 작은 그래프와 대형 그래프는 단일 배치로 유지해 지연을 제한한다.
- 폴더를 접을 때 직접 포함 파일의 상대 좌표를 저장하고, 접힌 폴더를 이동한 뒤 펼칠 때 해당 상대 좌표를 새 폴더 위치 기준으로 일괄 저장한다.
- 자동 정렬의 기본 배치 규모와 노드 간 최소 여백을 확대했다. 노드-간선 충돌 보정은 작은 그래프에만 적용하고 밀집 그래프에서는 spring 배치를 흐트러뜨리지 않도록 생략한다. 간선 교차 후처리는 다른 교차를 연쇄 생성해 적용하지 않는다.
- 같은 노드 쌍의 복수 관계는 관계 순서에 따라 대칭 평행 오프셋을 적용해 선과 라벨이 완전히 겹치지 않게 표시한다.
- 컨트롤 패널을 400px 고정 폭으로 설정하고 splitter 핸들을 잠가 반복적으로 오른쪽 면이 잘리는 문제를 차단했다.
- 컨트롤 패널 최소 폭을 넓혀 오른쪽 컨트롤과 내용이 잘리는 문제를 보완했다.
- 전체 그래프 재렌더링 시 현재 transform과 화면 중심을 복원해 일반 작업마다 화면 위치가 초기화되지 않도록 했다. 검색·부분 보기처럼 데이터 범위가 달라지는 화면은 새 범위에 맞춘다.
- 추천 관계 승인 후 생성된 관계의 출발·도착 노드 중앙으로 자동 이동한다.
- 후보 패널은 선택 노드로 필터링하지 않고 전체 대기 후보를 유지한다. 관계 없는 노드를 선택해도 추천이 사라지지 않는다.
